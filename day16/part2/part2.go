...
var tc = regexp.MustCompile(`(trees|cats)`)
var pg = regexp.MustCompile(`(pomeranians|goldfish)`)
...
func check(a map[string]int, s string) bool {
	if tc.Match([]byte(s)) && a[s] > sue[s] {
		return true
	} else if pg.Match([]byte(s)) && a[s] < sue[s] {
		return true
	} else if a[s] == sue[s] {
		return true
	}
